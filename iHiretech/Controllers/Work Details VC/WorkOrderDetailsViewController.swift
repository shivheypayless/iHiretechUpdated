...
class WorkOrderDetailsViewController: UIViewController , UIWebViewDelegate , URLSessionDownloadDelegate{
    ...
    @IBOutlet var cnstCheckoutHeight: NSLayoutConstraint!
    @IBOutlet var cnstTableTaskHeight: NSLayoutConstraint!
    ...
    @IBOutlet var cnstTaskHeight: NSLayoutConstraint!
    @IBOutlet var cnstStartLocationHeight: NSLayoutConstraint!
    @IBOutlet var btnStartLocation: UIButton!
    @IBOutlet var cnstCheckInHeight: NSLayoutConstraint!
   ...
   var socketId = String()
    ...
    {
        super.viewDidLoad()
        borderView(borderView: self.viewCheckInDate)
        borderView(borderView: self.viewCheckinTime)
        borderView(borderView: self.viewCheckOutDate)
        borderView(borderView: self.viewCheckOutTime)
        ...
        self.btnStartLocation.layer.cornerRadius = 3
        self.btnStartLocation.layer.masksToBounds = true
        ...
        self.tblTask.register(UINib(nibName: "TaskListTableViewCell", bundle: nil) , forCellReuseIdentifier: "TaskListTableViewCell")
...
//        }
    ...
    func keyboardWillShow(_ notification: Notification) {
        ...
        if let keyboardFrame: NSValue = notification.userInfo?[UIKeyboardFrameEndUserInfoKey] as? NSValue {
            ...
            {
                self.popup.frame.origin.y = self.popup.frame.origin.y - 120
                self.popup.show()
            }
            else
            {
    ...
    func getWorkList()
    {
        ...
        //    DispatchQueue.main.async {
            ...
            {
                self.cnstTableTaskHeight.constant = 0
            }
            else
            {
                self.cnstTableTaskHeight.constant = CGFloat(40 * (self.getWorkListData["work_order_tasks"] as! [AnyObject]).count)
                self.cnstTaskHeight.constant = 0
                self.viewTask.isHidden = true
                self.tblTask.delegate = self
                self.tblTask.dataSource = self
                self.tblTask.reloadData()
            }
            if !(self.getWorkListData["checkin_checkout"] is NSNull)
            {
            ...
            if ((information)["checkin_date"] as? String) != nil
            {
                ...
                let date = dateFormatter.date(from: ((information)["checkin_date"] as! String))
                dateFormatter.dateFormat = "yyyy-MM-dd hh:mm:ss Z"
                let finalDate = dateFormatter.string(from: date!)
                let dateFinal = dateFormatter.date(from: finalDate)
                dateFormatter.dateFormat = "MM-dd-yyyy"
                self.lblCheckInDate.text = dateFormatter.string(from: dateFinal!)
                self.imgCheckIn.image = UIImage(named : "img_CheckReminder")
            }
            else
            {
                self.imgCheckIn.image = UIImage(named : "img_UnCheckReminder")
            }
            if ((information)["checkin_time"] as? String) != nil
            {
                let dateFormatter = DateFormatter()
                dateFormatter.dateFormat = "HH:mm:ss"
                let date = dateFormatter.date(from: ((information)["checkin_time"] as! String))
                dateFormatter.dateFormat = "hh:mm a"
                self.lblCheckInTime.text = dateFormatter.string(from: date!)
                //(headerViewArray.viewWithTag(5) as! UILabel).text = ((information)["checkin_time"] as? String)!
            }
            if ((information)["checkin_date"] as? String) == nil
            {
                self.cnstStartLocationHeight.constant = 0
            }
                if ((information)["checkout_date"] as? String) != nil
                {
                    let dateFormatter = DateFormatter()
                    dateFormatter.dateFormat = "yyyy-MM-dd"
                    let date = dateFormatter.date(from: ((information)["checkout_date"] as! String))
                    dateFormatter.dateFormat = "yyyy-MM-dd hh:mm:ss Z"
                    let finalDate = dateFormatter.string(from: date!)
                    let dateFinal = dateFormatter.date(from: finalDate)
                    dateFormatter.dateFormat = "MM-dd-yyyy"
                    self.lblCheckOutDate.text = dateFormatter.string(from: dateFinal!)
                    self.imgCheckOut.image = UIImage(named : "img_CheckReminder")
                }
                ...
                if ((information)["checkout_time"] as? String) != nil
                {
                    let dateFormatter = DateFormatter()
                    dateFormatter.dateFormat = "HH:mm:ss"
                    let date = dateFormatter.date(from: ((information)["checkout_time"] as! String))
                    dateFormatter.dateFormat = "hh:mm a"
                    self.lblCheckOutTime.text = dateFormatter.string(from: date!)
                    
                }
            }
            if ((self.getWorkListData)["status_name"] as! String) == "approved" || ((self.getWorkListData)["status_name"] as! String) == "paid"
            {
                self.cnstCheckInHeight.constant = 0
                self.cnstCheckoutHeight.constant = 0
                self.cnstStartLocationHeight.constant = 0
            }
    ...
    @IBAction func btn_CheckInDate(_ sender: UIButton) {
        ...
        self.calenderPickerView.minimumDate = Date()
    ...
    @IBAction func btn_CheckInTime(_ sender: UIButton) {
        view.endEditing(true)
        var calenderView = UIView()
        calenderView = Bundle.main.loadNibNamed("DatePickerView", owner: self, options: nil)?[0] as! UIView
        calenderView.layer.cornerRadius = 10.0
    ...
    @IBAction func btn_Checkin(_ sender: UIButton) {
        if self.lblCheckInDate.text! != "Select Date" && self.lblCheckInTime.text! != "Select Time"
        {
            self.imgCheckIn.image = UIImage(named : "img_CheckReminder")
            ...
            let parameters = ["work_order_id": self.workOrderId ,"checkin_date": self.lblCheckInDate.text!,"checkin_time":self.lblCheckInTime.text! ] as [String:AnyObject]
             self.cnstStartLocationHeight.constant = 40.0
        ...
        {
            if self.lblCheckInDate.text! == "Select Date"
            {
                let viewShaker = AFViewShaker(view: self.viewCheckInDate)
                self.viewCheckInDate.layer.borderColor = UIColor.red.cgColor
                viewShaker?.shake()
                return
            }
            else if self.lblCheckInTime.text! == "Select Time"
            {
                let viewShaker = AFViewShaker(view: self.viewCheckinTime)
                self.viewCheckinTime.layer.borderColor = UIColor.red.cgColor
                viewShaker?.shake()
            }
            else
            {
    ...
    @IBAction func btn_StartUpdateLocation(_ sender: UIButton) {
        
        if self.btnStartLocation.titleLabel?.text == "Start Location Tracking"
        {
            self.btnStartLocation.setTitle("Pause Location Tracking", for: .normal)
        ...
        {
            self.btnStartLocation.setTitle("Start Location Tracking", for: .normal)
            self.StopSendingLocation()
        }
    }
    
    @IBAction func btn_CheckOutDate(_ sender: UIButton) {
        view.endEditing(true)
        var calenderView = UIView()
        calenderView = Bundle.main.loadNibNamed("DatePickerView", owner: self, options: nil)?[0] as! UIView
        calenderView.layer.cornerRadius = 10.0
        ...
        self.calenderPickerView = (calenderView.viewWithTag(3)! as! UIDatePicker)
        self.calenderPickerView.datePickerMode = UIDatePickerMode.date
        if lblCheckInDate.text! != "Select Date"
        {
            let dateFormatter = DateFormatter()
            dateFormatter.dateFormat = "MM-dd-yyyy"
            let date = dateFormatter.date(from: lblCheckInDate.text!)
            self.calenderPickerView.minimumDate = Date()
        }
    ...
    @IBAction func btn_CheckOutTime(_ sender: UIButton) {
        view.endEditing(true)
        var calenderView = UIView()
        calenderView = Bundle.main.loadNibNamed("DatePickerView", owner: self, options: nil)?[0] as! UIView
        calenderView.layer.cornerRadius = 10.0
    ...
    @IBAction func btn_CheckOut(_ sender: UIButton) {
        self.imgCheckOut.image = UIImage(named : "img_CheckReminder")
        if self.lblCheckOutTime.text! == self.lblCheckInTime.text!
        {
            AListAlertController.shared.presentAlertController(message: "Checkout date should not be less than checkIn date", completionHandler: nil)
            return
        }
        
        if self.lblCheckOutDate.text! != "Select Date" && self.lblCheckOutTime.text! != "Select Time"
        {
            self.imgCheckOut.image = UIImage(named : "img_CheckReminder")
            ...
            let parameters = ["work_order_id": self.workOrderId ,"checkout_date": self.lblCheckOutDate.text!,"checkout_time":self.lblCheckOutTime.text!] as [String:AnyObject]
        ...
        {
            if self.lblCheckOutDate.text! == "Select Date"
            {
                let viewShaker = AFViewShaker(view: self.viewCheckOutDate)
                self.viewCheckOutDate.layer.borderColor = UIColor.red.cgColor
                viewShaker?.shake()
                return
            }
            else if self.lblCheckOutTime.text! == "Select Time"
            {
                let viewShaker = AFViewShaker(view: self.viewCheckOutTime)
                self.viewCheckOutTime.layer.borderColor = UIColor.red.cgColor
                viewShaker?.shake()
            }
            else
            {
...
extension WorkOrderDetailsViewController : UITableViewDelegate , UITableViewDataSource
{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
       ...
       if self.tabsTag == 1
       {
        if indexPath.section == 0
        {
            ...
            cell.lblTypeOfWork.text = self.workOrderType
            ...
            {
                cell.cnstapplyHeight.constant = 0
            }
            else
            {
        ...
        else if indexPath.section == 4
        {
            ...
            if !((self.getWorkListData)["schedule_exact_date"] is NSNull)
            {
                ...
                let dateFinal = dateFormatter.date(from: finalDate)
                dateFormatter.dateFormat = "MM-dd-yyyy"
                cell.lblDate.text = dateFormatter.string(from: dateFinal!)
     ...
     //       self.sendMessageId = (chatHistory[indexPath.row]["to_id"] as? String)!
       ...
       else if self.tabsTag == 4
       {
        ...
        if indexPath.section == 0
        {
            ...
            cell.lblTotalExpenses.text = "$ \((self.chatDetails)["totalTechnicianPayment"] as! String)"
            cell.lblTotalWOrkingHrs.text = "\((self.chatDetails)["total_hours"] as! Double) hrs"
            if ((self.getWorkListData)["status_name"] as! String) == "approved" || ((self.getWorkListData)["status_name"] as! String) == "paid"
            {
               cell.cnstAddExpensesHeight.constant = 0
            }
            else
            {
        ...
        else if indexPath.section == 2
        {
            ...
            else
            {
                ...
                if !(((self.chatDetails)["blender"] as! [String:Any])["maxHours1"] is NSNull)
                {
                  cell.lblFirstMaxHours.text = String(((self.chatDetails)["blender"] as! [String:Any])["maxHours1"] as! Double)
                }
                if !(((self.chatDetails)["blender"] as! [String:Any])["maxHours2"] is NSNull)
                {
                  cell.lblsecndMaxhrs.text = String(((self.chatDetails)["blender"] as! [String:Any])["maxHours2"] as! Double)
                }
                if !(((self.chatDetails)["blender"] as! [String:Any])["clockedHours1"] is NSNull)
                {
                cell.lblFirstClockedHrs.text = String(((self.chatDetails)["blender"] as! [String:Any])["clockedHours1"] as! Double)
                }
                if !(((self.chatDetails)["blender"] as! [String:Any])["clockedHours2"] is NSNull)
                {
                  cell.lblScndClocedHrs.text = String(((self.chatDetails)["blender"] as! [String:Any])["clockedHours2"] as! Double)
                }
                if !(((self.chatDetails)["blender"] as! [String:Any])["amountEarned1"] is NSNull)
                {
                cell.lblFirstAmtEarnd.text = String(((self.chatDetails)["blender"] as! [String:Any])["amountEarned1"] as! Double)
                }
                if !(((self.chatDetails)["blender"] as! [String:Any])["amountEarned2"] is NSNull)
                {
        ...
        else
        {
            ...
            {
                if ((self.chatDetails)["fee"] as! Double) == 0
                {
                    cell.lblFees.text = "$ 0.00"
                }
                else
                {
       ...
       else
       {
        ...
        else if tableView == tblTask
        {
            ...
            cell.lblTask.text = (((self.getWorkListData["work_order_tasks"] as! [AnyObject])[indexPath.row])["task_name"] as! String)
            cell.btnCompleteTask.tag = indexPath.row
            if (((self.getWorkListData["work_order_tasks"] as! [AnyObject])[indexPath.row])["task_status"] as! Bool) == true
            {
                cell.imgCheckUnCheck.image = #imageLiteral(resourceName: "img_CheckReminder")
            }
            else
            {
                cell.imgCheckUnCheck.image = #imageLiteral(resourceName: "img_UnCheckReminder")
            }
            if ((self.getWorkListData)["status_name"] as! String) == "paid"
            {
                cell.btnCompleteTask.isHidden = true
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if tableView != self.tblListing && tableView != self.tblTask
        {
...
//            let date = dateFormatter.date(from: ((information)["checkin_date"] as! String))
//            dateFormatter.dateFormat = "MM-dd-yyyy"
//            (headerViewArray.viewWithTag(3) as! UILabel).text = dateFormatter.string(from: date!)
...
//            let date = dateFormatter.date(from: ((information)["checkin_time"] as! String))
//            dateFormatter.dateFormat = "hh:mm a"
//         //   (headerViewArray.viewWithTag(5) as! UILabel).text = dateFormatter.string(from: date!)
//            //(headerViewArray.viewWithTag(5) as! UILabel).text = ((information)["checkin_time"] as? String)!
//        }
//        if ((information)["checkin_date"] as? String) != nil
//        {
//             (headerViewArray.viewWithTag(9) as! UIButton).isHidden = false
//        }
//        else
//        {
//            (headerViewArray.viewWithTag(9) as! UIButton).isHidden = true
//        }
       if ((self.getWorkListData)["status_name"] as! String) == "approved" || ((self.getWorkListData)["status_name"] as! String) == "paid"
       {
        (headerViewArray.viewWithTag(2) as! UIButton).isHidden = true
        (headerViewArray.viewWithTag(9) as! UIButton).isHidden = true
       }
       else
       {
        (headerViewArray.viewWithTag(2) as! UIButton).isHidden = false
        (headerViewArray.viewWithTag(9) as! UIButton).isHidden = false
       }
    ...
    func tableView(_ tableView: UITableView, viewForFooterInSection section: Int) -> UIView? {
        ...
        if self.tabsTag == 4 && section == 1
        {
           let headerViewArray = Bundle.main.loadNibNamed("ExpensesFooterView", owner: self, options: nil)?[0] as! UIView
            (headerViewArray.viewWithTag(1) as! UIView).layer.borderWidth = 1
            (headerViewArray.viewWithTag(1) as! UIView).layer.borderColor = UIColor(red: 221/255, green: 221/255, blue: 221/255, alpha: 1).cgColor
            if (self.chatDetails)["total_expenses"] as! Double == 0
            {
                (headerViewArray.viewWithTag(2) as! UILabel).text = "$ 0.00"
            }
            else
            {
            (headerViewArray.viewWithTag(2) as! UILabel).text = "$ \((self.chatDetails)["total_expenses"] as! Double)"
            }
            if ((self.chatDetails)["total_expenses_excess"] as! Double) == 0
            {
                (headerViewArray.viewWithTag(3) as! UILabel).text = "$ 0.00"
            }
            else
            {
...
//            }
    ...
    func tableView(_ tableView: UITableView, heightForHeaderInSection section: Int) -> CGFloat
    {
         ...
         if self.tabsTag == 4 && section == 1
        {
            ...
            else if self.tabsTag == 3
         {
            ...
            {
            let information = self.getWorkListData["checkin_checkout"] as! [String:Any]
            if !((information)["checkin_date"] is NSNull)
            {
               return 305.0
            }
            else
   ...
   func tableView(_ tableView: UITableView, heightForFooterInSection section: Int) -> CGFloat
    ...
    @objc func btn_SaveDateToOutAction(_ sender: UIButton)
    {
        ...
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = "MM-dd-yyyy"
        self.lblCheckOutDate.text! = dateFormatter.string(from: calenderPickerView.date)
    }
    ...
    @objc func DatePickerToTimeCheckOut(_ sender : UIButton)
    {
        view.endEditing(true)
        var calenderView = UIView()
        calenderView = Bundle.main.loadNibNamed("DatePickerView", owner: self, options: nil)?[0] as! UIView
    ...
    @objc func btn_SaveTimeOutAction(_ sender: UIButton)
    {
        ...
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = "hh:mm a"
        self.lblCheckOutTime.text! = dateFormatter.string(from: calenderPickerView.date)
    }
    ...
    @objc func btn_SaveDateToInAction(_ sender: UIButton)
    {
        ...
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = "MM-dd-yyyy"
        self.lblCheckInDate.text! = dateFormatter.string(from: calenderPickerView.date)
    }
    ...
    @objc func btn_SaveTimeInAction(_ sender: UIButton)
    {
        ...
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = "hh:mm a"
        self.lblCheckInTime.text! = dateFormatter.string(from: calenderPickerView.date)
    }
    ...
    {
        if self.expensesTable.isHidden == true
        {
    ...
    @objc func btn_SubmitExpenses(_ sender : UIButton)
    {
        ...
        else if self.expenseText.text! != "" && self.selectExpenses.titleLabel?.text! != "  Select"
       {
         ...
         let parameters = ["work_order_id": self.workOrderId ,"expense_type": (self.selectExpenses.titleLabel?.text!)!,"expense_amount":Float(self.expenseText.text!)! , "expense_description":self.expensesDescription.text!] as [String:AnyObject]
  ...
  func startUpdatingLocation()
  {
    ...
    @objc func btn_CompletedtaskList(_ sender: UIButton)
    {
        ...
        WebAPI().callJSONWebApi(API.completedTaskList, withHTTPMethod: .post, forPostParameters: parameters, shouldIncludeAuthorizationHeader: true, actionAfterServiceResponse: { (serviceResponse) in
            ...
            if let message = serviceResponse["msg"] as? String
            {
                AListAlertController.shared.presentAlertController(message: message, completionHandler: {
                    self.getWorkList()
                })
            }
        })
    ...
    @objc func CustomerRatingDetail(_ sender: UIButton)
    {
        ...
        destination.customerId = ((self.getWorkListData)["customer_id"] as! Int)
...
//        }
    ...
    func borderView(borderView : UIView)
    {
        ...
        borderView.layer.borderColor = UIColor(red: 221/255, green: 221/255, blue: 221/255, alpha: 1).cgColor
    }
